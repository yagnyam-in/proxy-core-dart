...
typedef SignableMessageFromJson = SignableMessage Function(Map<String, dynamic>);
...
class MessageFactory with ProxyUtils {
  ...
  MessageFactory({
    ...
    @required this.messageVerificationService,
  }) {
  ...
  SignedMessage<T> buildSignedMessage<T extends SignableMessage>(
    ...
    SignedMessage signedMessageObject = SignedMessage.fromJson(jsonDecode(jsonMessage));
    signedMessageObject.message = signableMessageFactoryMethod(jsonDecode(signedMessageObject.payload));
    return signedMessageObject;
  }

  Future<bool> verifySignedMessage<T extends SignableMessage>(SignedMessage<T> signedMessage) async {
  ...
  Future<SignedMessage<T>> buildAndVerifySignedMessage<T extends SignableMessage>(
      String jsonMessage, FromJson<T> signableMessageFactoryMethod) async {
    SignedMessage<T> signedMessage = buildSignedMessage(jsonMessage, signableMessageFactoryMethod);
